...
var MoengageSW = (function(self) {
  ...
  var moeReportParamsDB, moeCampaignsDB, moeOfflineDB;
  ...
  async function initDB() {
    return new Promise(resolve => {
      moeReportParamsDB = localforage.createInstance({
        driver: [localforage.INDEXEDDB],
        name: 'moe_database',
        storeName: 'moe_data',
      });
      moeCampaignsDB = localforage.createInstance({
        driver: [localforage.INDEXEDDB],
        name: 'moe_database',
        storeName: 'moe_backup',
      });
      moeOfflineDB = localforage.createInstance({
        driver: [localforage.INDEXEDDB],
        name: 'moe_database',
        storeName: 'offline_data',
      });
      Promise.all([
        moeReportParamsDB.ready(),
        moeCampaignsDB.ready(),
        moeOfflineDB.ready(),
      ]).then(resolve());
    });
  }
  ...
  function onActivate(event) {
    console.log('onActive event', event?.data);

    event.waitUntil(self.clients.claim());
  }

  function onMessage(event) {
    console.log('mesage event', event?.data);
    if (event.data) {
      if (event.data && event.data.app_id) {
        moeReportParamsDB.setItem('reportParams', event);
      ...
      moeReportParamsDB
        ...
        .then(async function(data) {
          ...
          if (data.cid) {
            ...
            moeCampaignsDB.setItem(campaignId, campaignBackup);
          }
  ...
  function isCampaignAlreadyTriggered(campaignId) {
    return moeCampaignsDB
      .length()
      .then(numberOfKeys => {
        if (numberOfKeys === 0) {
          ...
          return moeCampaignsDB
            .iterate(function(campaign, dbCampaignId) {
  ...
  function removeOldCampaignsFromIndexedDB() {
    moeCampaignsDB.iterate(function(campaign, campaignId) {
      const now = Date.now();
  ...
  function onNotificationClick(event) {
    ...
    var eventTracker = new Promise(function(res) {
      moeCampaignsDB
        .iterate(function(campaign, campaignId) {
  ...
  function onNotificationClose(event) {
    var notificationData = event.notification.data;
    notificationData && moeCampaignsDB
      .iterate(function(campaign, campaignId) {
        if (campaignId == notificationData.cid) {
          //   moeCampaignsDB.removeItem(campaignId);
          return campaignId;
  ...
  function trackEvent(eventName, attrs, flag, extraKeys = {}) {
    moeReportParamsDB.getItem('reportParams').then(function(res) {
      var data = res.data;
      // var postData = {
      //   e: eventName,
      //   a: attrs,
